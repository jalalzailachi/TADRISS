        ...
        <div className="p-6 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <a href="https://docs.tadriss.com" target="_blank" className="p-6 bg-surface-container-low rounded-3xl border border-outline-variant/10 hover:border-primary transition-all group">
              ...
              <h4 className="text-sm font-black text-on-surface uppercase tracking-tight mb-1">{t('kbTitle')}</h4>
              <p className="text-[10px] text-outline font-bold uppercase tracking-widest opacity-60">{t('kbDesc')}</p>
            </a>
            <a href="mailto:support@tadriss.com" className="p-6 bg-surface-container-low rounded-3xl border border-outline-variant/10 hover:border-accent transition-all group">
              ...
              <h4 className="text-sm font-black text-on-surface uppercase tracking-tight mb-1">{t('contactTitle')}</h4>
              <p className="text-[10px] text-outline font-bold uppercase tracking-widest opacity-60">{t('contactDesc')}</p>
            </a>
          </div>

          <div className="p-6 bg-on-surface rounded-3xl relative overflow-hidden">
            <div className="relative z-10">
               <h4 className="text-white text-lg font-black uppercase tracking-tighter mb-2 italic">{t('onboardingTitle')}</h4>
               <p className="text-white/60 text-[10px] font-bold uppercase tracking-[0.2em] mb-6">{t('onboardingPhase')}</p>
               <div className="space-y-4">
                  <div className="flex items-center gap-3 text-white/80">
                    <span className="material-symbols-outlined text-primary">check_circle</span>
                    <span className="text-[10px] font-black uppercase tracking-widest">{t('step1')}</span>
                  </div>
                  <div className="flex items-center gap-3 text-white/40">
                    <span className="material-symbols-outlined">circle</span>
                    <span className="text-[10px] font-black uppercase tracking-widest">{t('step2')}</span>
                  </div>
               </div>
            </div>
