...
export async function addStudent(formData: FormData) {
  try {
    ...
    if (authError) {
      console.error('[addStudent] createUser error:', authError)
    ...
    if (profileError) {
      console.error('[addStudent] profile insert error:', profileError)
    ...
    console.error('[addStudent] unexpected:', err)
...
export async function updateStudent(studentId: string, formData: FormData) {
  try {
    ...
    if (!updated || updated.length === 0) return { error: 'Élève non trouvé dans votre établissement.' }
    ...
    if (authError) console.error('[updateStudent] auth update error:', authError)
...
export async function deleteStudent(studentId: string) {
  try {
    ...
    // Verify student belongs to this institution before acting
    const { data: student } = await adminClient.from('profiles')
      .select('id')
      .eq('id', studentId)
      .eq('institution_id', adminProfile.institution_id)
      .single()

    if (!student) return { error: 'Élève non trouvé dans votre établissement.' }
    ...
    await adminClient.auth.admin.updateUserById(studentId, { ban_duration: '876000h' })
